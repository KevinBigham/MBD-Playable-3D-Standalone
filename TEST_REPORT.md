...
| `bookkeeping.test.ts` | 7 | 120 games: result never snapshotted before the winning play is credited, every run charged to exactly one pitcher, RBI never exceed runs, home runs consistent between batting and pitching lines, derby swing-off crowns a participant |
...
| `bridge.test.ts` | 45 | The MBD arcade-world bridge, tested against the contract's own safety rules. The published rating conversion reproduced exactly; a monotonicity sweep across all 551 internal values and across both inputs of the one blended rating; proof that a corrupt `arcade99` changes nothing about play because only `internal` is read. Park factor resolving to the nearest park by carry for all 32 franchises, with Denver and San Francisco landing the right way round. Eight rejection cases that must not be repaired — duplicate ids, a roster claiming another club's player, an 8-man lineup, a hitter batting twice, an ineligible bat, a starter who cannot pitch, an unavailable player in a game, a duplicated modifier. Then a real MBD matchup played to completion through the actual engine, a receipt that reconciles against its own package, and four receipts that must not: a doctored final, a foreign bundle hash, a player who was never eligible, and an exhibition package, which produces no receipt at all. Then four that the loaded league reaches every mode: cycling stays inside it and comes back where it started, the avoid-this-club rule holds in both directions, all 32 imported clubs get their own park rather than a shared default, and a Meridian club id still finds its ballpark while an MBD world is loaded. Finally the world that actually ships \u2014 `public/mbd-world.json`, MBD's real opening day \u2014 validated as data: it passes the same validation an import would, carries all 32 franchises, contains MBD's own players with self-consistent ratings, exports none of the scouting truth the contract forbids, and fields nobody who does not bat |
